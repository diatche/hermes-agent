...
class NativeTodoAgent:
    """Emits local-todo-shaped events while Slack native cards are enabled."""
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None):
        self.tool_progress_callback(
            ...
            "delegate_task",
            "delegating 1 task",
            {"goal": "Review Slack rendering"},
        )
        self.tool_start_callback("todo-1", "todo", {"merge": False})
        self.tool_complete_callback(
            "todo-1", "todo", {"merge": False}, '{"todos": []}'
        )
        time.sleep(0.35)
        return {"final_response": "done", "messages": [], "api_calls": 1}
...
def test_combined_tool_complete_callback_prefers_native_slack_cards():
    ...
    calls = []
    runner.todo_complete_callback = lambda *args: calls.append(("todo", args))
    runner.native_tool_complete_callback = lambda *args: calls.append(("native", args))

    runner.combined_tool_complete_callback(
        "call-1", "todo", {"merge": False}, '{"todos": []}'
    )

    assert calls == [
        ("native", ("call-1", "todo", {"merge": False}, '{"todos": []}'))
    ]
...
@pytest.mark.asyncio
async def test_slack_native_cards_suppress_local_todo_progress(monkeypatch, tmp_path):
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        NativeTodoAgent,
        session_id="sess-native-no-local-todo",
        config_data={"display": {"todo_progress": True}},
        platform=Platform.SLACK,
        chat_id="C1",
        thread_id="thread-1",
        adapter_cls=NativeTaskCardAdapter,
