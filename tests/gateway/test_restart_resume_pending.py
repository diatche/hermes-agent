...
class TestResumePendingSystemNote:
    ...
    def test_resume_pending_note_warns_against_reexecuting_restart(self):
        ...
        assert "Address the user's NEW message below FIRST" in result
        assert "resume the unfinished goal" in result
        assert "unless the new message supersedes it" in result
    ...
    def test_resume_pending_empty_message_continues_unfinished_goal(self):
        ...
        the next safe unfinished step rather than stall for instructions.
        """
        entry = self._pending_entry(reason="restart_timeout")
        result = _simulate_note_injection(
            history=[
        ...
        assert "continue the unfinished goal" in result
        assert "next safe incomplete step" in result
        assert "Ask the user only if" in result
        assert "do NOT re-execute or verify" in result
        assert "authoritative prior results, including failures" in result
