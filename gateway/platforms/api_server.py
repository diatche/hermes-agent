...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _parse_model_routes(raw: Any) -> Dict[str, Dict[str, Any]]:
        ...
        reasoning_effort?, skip_context_files?}``.
    ...
    def _create_agent(
        ...
        route_options = route if isinstance(route, dict) else {}
        route_runtime_options_active = False
        ...
        if session_override:
            ...
            route_runtime_options_active = route is not None
        ...
        # Route-level agent construction options are defaults for that route,
        # not properties of a session identity. Apply them only when the route
        # won the selection above. Explicit request model_options keep their
        # higher, request-scoped precedence.
        if (
            route_runtime_options_active
            and request_reasoning_config is None
            and route_options.get("reasoning_effort")
        ):
            from hermes_constants import parse_reasoning_effort

            route_reasoning = parse_reasoning_effort(
                route_options["reasoning_effort"]
            )
            if route_reasoning is not None:
                reasoning_config = route_reasoning
        ...
        agent_kwargs = {
            ...
            **_checkpoint_agent_kwargs(user_config),
            ...
            "skip_context_files": bool(
                route_runtime_options_active
                and route_options.get("skip_context_files")
            ),
