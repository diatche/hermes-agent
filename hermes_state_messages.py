...
def _commit_compaction_stage(db, target_session_id: str, stage_session_id: str, lock_holder: str, *,
    ...
    def _commit(conn):
        ...
        # Resolve exact originals before archiving changes the ACTIVE set used by identity matching.
        rewind_ids: List[int] = db._resolve_carried_row_ids(
            conn, target_session_id, carried_messages or [])
        if tail_count > 0:
        ...
        rewind_ids = list(dict.fromkeys([*rewind_ids, *tail_ids]))
...
class SessionMessagesMixin:
    ...
    def archive_and_compact(self, session_id: str, compacted_messages: List[Dict[str, Any]],
        ...
        # Staging stamps each inserted dict with the stage row id. Preserve the target-session
        # identities needed to classify exact carried originals at final cutover.
        carried_snapshot = [dict(message) for message in (carried_messages or [])
        ...
        try:
            ...
            inserted = _commit_compaction_stage(
                ...
                carried_messages=carried_snapshot)
            logger.info(
